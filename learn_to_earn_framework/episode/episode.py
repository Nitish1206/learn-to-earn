...
class Episode(object):
    ...
    def __init__(self, config_, episode_id, ind_exploration_factor, hex_attr_df, hex_distance_df,
        ...
        # Create drivers
        self.drivers = {}
        strategic_drivers = np.random.choice(range(self.num_drivers), self.num_strategic_drivers, replace=False)
        ddm = self.DT.initialize_driver_distribution()
        for hex_bin in range(S):
    ...
    def execute_driver_actions(self, t, city_state, driver_actions):
        ...
        driver_earnings = {}
        for idx in range(len(driver_actions)):
    ...
    def run(self):
        ...
        for t in range(T):
            ...
            vec_driver_actions = vec_choose_driver_action(range(self.num_drivers), t, best_actions)
